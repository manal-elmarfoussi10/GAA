    ...
    <form action="{{ route('users.update', $user->id) }}" method="POST" class="space-y-4">
        ...
        <div>
            <label class="block font-medium text-sm text-gray-700">
                Mot de passe <span class="text-sm text-gray-400">(laisser vide pour ne pas changer)</span>
            </label>
            <input type="password" name="password"
                   class="w-full border-gray-300 rounded shadow-sm focus:ring-orange-500 focus:border-orange-500 @error('password') border-red-500 @enderror">
        </div>
        ...
        <div>
            ...
            <select name="role"
                ...
                <option value="client_support" {{ old('role', $user->role) === 'client_support' ? 'selected' : '' }}>Service client</option>
                <option value="limited_client_support" {{ old('role', $user->role) === 'limited_client_support' ? 'selected' : '' }}>Service client limité</option>
                <option value="commercial" {{ old('role', $user->role) === 'commercial' ? 'selected' : '' }}>Commercial</option>
                <option value="service_devis" {{ old('role', $user->role) === 'service_devis' ? 'selected' : '' }}>Service Devis, commande et RDV</option>
                <option value="poseur" {{ old('role', $user->role) === 'poseur' ? 'selected' : '' }}>Poseur</option>
