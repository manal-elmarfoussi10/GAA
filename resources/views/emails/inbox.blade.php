        ...
        <div class="bg-white rounded shadow overflow-hidden">
            @forelse($emails as $email)
                <a href="{{ route('emails.show', $email->id) }}"
                   class="block group px-6 py-4 border-b hover:bg-gray-50 transition">
                    <div class="flex justify-between items-center">
                        {{-- Left: Star + Subject + Tag --}}
                        <div class="flex items-center gap-4">
                            {{-- Toggle important --}}
                            <form method="POST" action="{{ route('emails.toggleImportant', $email->id) }}"
                                  onclick="event.stopPropagation();">
                                @csrf
                                <button type="submit" title="Marquer comme important" class="focus:outline-none">
            ...
            @empty
                <div class="text-center text-gray-500 py-10">Aucun email dans la boîte de réception.</div>
            @endforelse
        </div>
    </div>
</div>
