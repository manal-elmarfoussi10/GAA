...
<div class="px-8 py-10">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Modifier le devis</h1>
    ...
    <form action="{{ route('devis.update', $devis->id) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div>
                ...
                <input type="text" name="titre" value="{{ $devis->titre }}" class="w-full border border-gray-300 rounded px-4 py-2" placeholder="Titre du devis">
            </div>

            <div>
                <label class="block mb-1 font-medium text-gray-700">Date du devis</label>
                <input type="date" name="date_devis" value="{{ $devis->date_devis }}" class="w-full border border-gray-300 rounded px-4 py-2">
            </div>

            <div>
                <label class="block mb-1 font-medium text-gray-700">Valide jusqu'au</label>
                <input type="date" name="date_validite" value="{{ $devis->date_validite }}" class="w-full border border-gray-300 rounded px-4 py-2">
            </div>
        </div>
